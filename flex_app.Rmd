...
absolutePanel(class = "panel panel-default", fixed = TRUE,
                  draggable = FALSE, top = '70%', left = '1%', right = 'auto', bottom = 'auto',
                  width = '20%',
                  style = "opacity: 1",
        ...
        tags$div(id = 'box6', class="collapse", style = "max-height: 420px",
            wellPanel(
                 ...
                 dateRangeInput("date_trend", label = NULL, "yyyy-mm-dd",
                                language = "cs", weekstart = 1, startview = "decade", separator = "do",
...
if(input$indikatory == "spi"){
    spi_f <- ind_dta[variable == input$entry.variable.ind]
    dta <- (sp::merge(povodi,spi_f[year == year(input$entry.date.ind) & month==month(input$entry.date.ind)], by="UPOV_ID"))}
  else{
    spei_f <- ind_dta
    dta <- (sp::merge(povodi,ind_dta[year == year(input$entry.date.ind) & month==month(input$entry.date.ind)], by="UPOV_ID"))}
...
# (- 2,0 až -inf) - tmave červena
  ...
  return(list(dta=dta, spi_f = spi_f, spei_f = spei_f, pal = pal))
}, ignoreNULL = FALSE, ignoreInit = FALSE)
...
renderDygraph({
  ...
  if(input$indikatory == "spi"){
    ts.indikatory <- reac.ind()$spi_f[UPOV_ID == default.shape.ind()] %>% select(value, DTM)
  }else{
    ts.indikatory <- reac.ind()$spei_f[UPOV_ID == default.shape.ind()] %>% select(value, DTM)
  }
