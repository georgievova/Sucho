...
labels.points <- sprintf(
      ...
      u.filtr$NAZICO, u.filtr$POVODI, u.filtr$NAZTOK, u.filtr$JEV
      ) %>% lapply(htmltools::HTML)
...
pal_2 <- colorFactor("Paired", domain = u.filtr$JEV, na.color = "gray50")
...
leaflet() %>%
    ...
    addPolygons(data=povodi.ds(), 
                ...
                labelOptions = labelOptions(clickable = TRUE, style = list("font-weight" = "normal",
                                             ...
                                             "border-color" = "rgb(255, 255, 255)"),
                                             textsize = "15px", direction = "auto")) %>% 
    addCircleMarkers(data=xy[kde[,1],], radius = 5, color = "black", fillColor = pal_2(u.filtr$JEV),
    ...
    addLayersControl(overlayGroups = c("Mapový podklad", "Řeky", "Jezera"),
                     position = "bottomleft",
                     options = layersControlOptions(collapsed = FALSE, autoZIndex = FALSE)) %>% 
    addLegend(pal = pal_2, values =u.filtr$JEV, opacity = 0.7, title = "Legenda",
              position = "bottomright")
