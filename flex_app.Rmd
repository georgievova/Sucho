...
renderDygraph({
  ...
  ts.bilance <- BM %>% filter(UPOV_ID == default.shape(), variable == input$entry.variable2) %>% select(value, DTM) 
...
rhandsontable(DF, width = 500, height = 600) %>%
...
renderDygraph({
  ts.uzivani.uv <-  u %>% filter(NAZICO == default.circle_uv()) %>% group_by(JEV, DTM) %>% ungroup %>% dcast(DTM~JEV)
