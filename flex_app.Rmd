...
xy <- kde()$xy
  ...
  labels <- sprintf(
      ...
      povodi$NAZ_UTVAR, povodi$KTG_UPOV, povodi$U_PMU, povodi$UPOV_ID) %>% lapply(htmltools::HTML)
...
uzivani <- leaflet() %>%setView(lng=centr[,1], lat = centr[,2], zoom=12) %>%
    ...
    addPolygons(data=povodi, color = "#2F4F4F", fillColor = "#F5F5F5", weight = 2, opacity = 0.4,
                          fillOpacity = 0.7, layerId = povodi$UPOV_ID,
                ...
                labelOptions = labelOptions(clickable = TRUE, style = list("font-weight" = "normal",
                                             ...
                                             "border-color" = "rgb(255, 255, 255)"),
                                             textsize = "15px", direction = "auto")) %>%
    addCircleMarkers(data=xy, radius = 5, color = "black", fillColor = pal_2(u_leaflet$JEV),
