...
dta_tr <- BM[UPOV_ID == default.shape() &
  ...
  ts.season <- dta_tr %>% select(seasons, value, DTM) %>%  
    filter(year(DTM) > input$date_trend[1] & year(DTM) < input$date_trend[2])
  
  ts.season$t <- (1:length(ts.season$value))
  ts.season$seasons <- as.factor(ts.season$seasons)
  
  model_s <- lm(value~t+seasons-1,data=ts.season)
  ts.season$lmt <- model_s$fitted.values
  
  return(list(ts.year=ts.year, ts.month=ts.month, ts.season=ts.season,
              model_r=model_r, model_m=model_m, model_s=model_s))
})
...
renderDataTable({
  ...
  model_r = trend.reac.app$model_r
  ...
  smr <- summary(model)
  p <- model$rank
  d <- smr$coefficients[,c(1,4)]
  dfr <- data.frame(model=c(type, rep(" ", (p-1))),
                    variables = gsub("seasons","",gsub("month2","",rownames(d))),
                    round(d,3),
                    R2 = round(as.numeric(c(smr$r.squared, rep("", (p-1)))),3)
                    )
  # rownames(dfr) <- NULL
  return(dfr)}
  
  rbind(sum.tab("roční", model_r),
        sum.tab("sezonní", model_s),
        sum.tab("měsíční", model_m)) #%>% formatRound(columns = c(3,4,5),3))
