...
reac<- eventReactive(input$go1,{ 
  ...
  }else{pars_filtr <- pars %>% filter(name == input$entry.pars)}
  
  dta.pars <- (sp::merge(povodi,pars_filtr, by="UPOV_ID"))
  dta <- (sp::merge(povodi,bilance_filtr, by="UPOV_ID"))
  
  return(list(bilance=bilance_filtr,dta=dta, dta.pars=dta.pars))
...
output$map<-renderLeaflet({
  ...
  map <- map %>% addPolygons(data=dta1, layerId = dta1$UPOV_ID, color = "#000066", 
    ...
    map <- map %>% addPolygons(data=dta.pars, layerId = dta.pars$UPOV_ID, color = "#ffffff", 
                             fillColor = ~pal.pars(current), fillOpacity = 0.8, group = "Povodí",
                 highlightOptions = highlightOptions(color = "#a53333", opacity = 1, fillOpacity = 0,
                                                     weight = 2.3,
                                                     bringToFront = TRUE, sendToBack = TRUE),
                 label = labels.pars,
...
renderDataTable({
  subtotal <- u %>% filter(UPOV_ID == default.shape2()) %>% replace(is.na(.), 0) %>% group_by(NAZICO) %>% summarise(suma = sum(value))}, width='50%', options = list(scrollY = "400px", scrollX = "10px", 
