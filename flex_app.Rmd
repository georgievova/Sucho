...
wellPanel(HTML('<div class="help-tip">
  ...
  radioButtons("indikatory", label=NULL, choices = c("SPI" = "spi","SPEI" = "spei"), 
...
# leaflet() %>% setView(lng=centr[,1], lat = centr[,2], zoom=12) %>%
  ...
  uzivani <- leaflet() %>% setView(lng=centr@coords[1], lat = centr@coords[2], zoom=12)
   ...
   if("MP" %in% vrstvy){
  uzivani <- uzivani %>% addProviderTiles(providers$Esri.WorldTopoMap, group = "Mapový podklad",
...
# leaflet() %>% setView(lng=centr[,1], lat = centr[,2], zoom=12) %>%
  ...
  if("MP" %in% vrstvy){
  vyhled_uzivani <- vyhled_uzivani %>% addProviderTiles(providers$Esri.WorldTopoMap, group = "Mapový podklad",
                                                  options = tileOptions(minZoom=7, maxZoom=20))}
          ...
          conditionalPanel(condition = "input.validace == 'A'",
                  absolutePanel(class = "panel panel-default", fixed = TRUE,
                                         ...
                                         tags$div(id = 'box8', class="collapse", style = "max-height: 200px;
                                                  overflow-y:scroll"),
                              conditionalPanel(condition = "input.validace == 'A'",
                                               checkboxGroupInput("entry.variable4", "Zvolte:", 
                                                                  choices = c("value", "mean_month", "mean_year"), 
          ...
          conditionalPanel(condition = "input.validace == 'AA'",
                           absolutePanel(class = "panel panel-default", fixed = TRUE,
                                         ...
                                         tags$div(id = 'box8', class="collapse", style = "max-height: 200px;
                                                  overflow-y:scroll"),
                                conditionalPanel(condition = "input.validace == 'AA'",
                                                 checkboxGroupInput("entry.variable5", "Zvolte:", 
                                                                    choices = choices3,
                                                                    selected = c("QMER", "RM"))))),
 ...
 conditionalPanel(condition = "input.validace == 'B'",
          ...
          wellPanel(checkboxGroupInput("entry.layer.val", label = "Vrstvy",
                                                        choices = c("Mapový podklad" = "MP", "Data" = "P",
                                                                    "Řeky" = "R", "Jezera" = "J", "Nádrže" = "N", "Kraje" = "K", 
...
output$validace <- renderLeaflet({
  ...
  validace <- leaflet() # addTiles(options = tileOptions(minZoom = 4, maxZoom = 7)) #%>% setView(lng=15,lat=50, zoom=5) 
    ...
    validace <- validace %>% addCircleMarkers(data = stanice, lng = stanice@coords[,1], lat = stanice@coords[,2], 
                                               radius = 3.5, color = "black", fillColor = "steelblue", weight = 0.5, opacity = 1, 
                                               fillOpacity = 1, popup = label.points, layerId = stanice$DBCN) %>%
  ...
  observe({
    click <- input$validace_shape_click
    x <- subset(povodi,  UPOV_ID==stanice$UPOV_ID[stanice$DBCN==click])
    # x2 <- subset(stanice, DBCN == default.circle_val())
   leafletProxy("validace") %>% 
    # clearBounds() %>% 
    clearGroup("current_upov") %>%
    addPolylines(data = x,
                 group = "current_upov",
                 color = "#a53333", opacity = 1, weight = 3) 
   
   # setView(lng=x@polygons[[1]]@labpt[1],
   #         lat=x@polygons[[1]]@labpt[2], 
   #         zoom=input$validace_zoom)
   # leafletProxy("validace") %>% 
   #   setView(lng=x@polygons[[1]]@labpt[1],lat=x@polygons[[1]]@labpt[2], zoom=18) 
   # %>% 
   #   addCircleMarkers(data = x2, lng = x2@coords[,1], lat = x2@coords[,2], 
   #                                             radius = 6, color = "black", fillColor = "red", weight = 1, opacity = 1, 
   #                                             fillOpacity = 1, popup = label.points, layerId = stanice$DBCN, group="current_shape") 
    })
...
renderPlotly({
  ...
  # routed_0_dir <- data.frame(source=d, UPOV_ID=gsub("*.rds","",d))
  # routed_0_dir <- readRDS(file.path(.datadir,"routed-0_bilan_bez_VN", routed_0_dir$source[routed_0_dir$UPOV_ID==default.shape3()]))
  # 
  # ts.year <- dta_bd %>%  select(input$variable_trend, DTM) %>% mutate(m = month(as.Date(DTM)))
  # colnames(ts.year)[1] <- "var" 
  # ts.year$t <- (1:length(ts.year$var))
    ...
    ts.QD <- QD %>% filter(DBCN == default.line()) %>% mutate(month = format(DTM, "%m"), year = format(DTM, "%Y")) %>%
        ...
        p1 <- plot_ly(data=ts.QD, text=paste(ts.QD$DTM), mode="lines", type="scatter", line = list(width = 1)) %>%
          layout(title=paste(as.character(title.f5())))
        
        if( "value" %in% input$entry.variable4){
          p1 <- p1 %>% add_trace(x = ~DTM, y = ~value, name = 'Denni prutoky')} 
        
        if( "mean_month" %in% input$entry.variable4){
          p1 <- p1 %>% add_trace(x = ~DTM, y = ~mean_month, name = 'Mesicni prumery')}
        
        if("mean_year" %in% input$entry.variable4){
          p1 <- p1 %>% add_trace(x = ~DTM, y = ~mean_year, name = 'Rocni prumery')}
        
        p1
  
  }else if(input$validace == "AA"){
  
        ts.dta.val <- dta_val %>% filter(DBCN == default.circle_val()) %>% select(input$entry.variable5, DTM)

        p <- plot_ly(data=ts.dta.val, text=paste(ts.dta.val$DTM), mode="lines", type="scatter", line = list(width = 1)) %>%
          layout(title=paste(as.character(title.f4())))
        
        if( "QMER" %in% input$entry.variable5){
          p <- p %>% add_trace(x = ~DTM, y = ~QMER, name = 'QMER')} 
        
        if( "QNEX" %in% input$entry.variable5){
          p <- p %>% add_trace(x = ~DTM, y = ~QNEX, name = 'QNEX')}
        
        if("QNEY" %in% input$entry.variable5){
          p <- p %>% add_trace(x = ~DTM, y = ~QNEY, name = 'QNEY')}
        
        if("RM" %in% input$entry.variable5){
   ...
   p2 <- plot_ly(data=chars_filter, x=~m, y=~value, type="scatter", mode="lines") %>%
...
if(input$validace == "A"){
  ...
  # x <- rbind(c(colnames(t1),rep(" ",9)),cbind(t1,matrix(rep(NA, 4*9), nrow=4)))
  # colnames(x) <- colnames(t2)
  # print(rbind(t2, x))
  
  # dta_val_filter <-cbind(name="value", dta_val_filter[,-1])
  # else if(input$validace == "B"){
