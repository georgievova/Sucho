...
kde <- reactive({
u_leaflet <- u %>% group_by(NAZICO, POVODI, JEV, X, Y) %>% summarise(mean=mean(value))
...
labels.points <- sprintf(
      "<strong>%s</strong><br/>Povodí: %s<br/>Jev: %s",
      u_leaflet$NAZICO, u_leaflet$POVODI, u_leaflet$JEV) %>%
      lapply(htmltools::HTML)
