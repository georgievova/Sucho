...
wellPanel(
    HTML('<div class="help-tip">
          ...
          conditionalPanel(condition = "input.validace == 'A'",
                           wellPanel(checkboxGroupInput("entry.layer.valA", label = NULL,
                                                        choices = c("DBCN body" = "DB"), 
                                                        selected = c("DB"), inline = T))),
...
output$validace <- renderLeaflet({
  ...
  if(input$validace == "A"){
  # label.lines <- sprintf(
  #     "<strong>%s</strong><br/> UPOV_ID:%s",
  #     povodi$NAZ_UTVAR, povodi$UPOV_ID) %>% 
  #     lapply(htmltools::HTML)
  
  dta_val2 <- dta_val %>% group_by(NAZ, DBCN, UPOV_ID) %>% summarise(n=n())    
  
  label.points <- sprintf(
      "<strong>%s</strong><br/> DBCN:%s<br/> UPOV_ID:%s",
      dta_val2$NAZ, dta_val2$DBCN, dta_val2$UPOV_ID) %>% 
      lapply(htmltools::HTML)
  
 #Zbarvení povodí dle hodnoty + Legenda
  
    # if("P" %in% vrstvy){}
    # validace <-  validace %>% addPolylines(data=povodi, color="#000000", weight = 1.2, fill = TRUE, fillOpacity = 0.01,
    #              opacity = 1, stroke= TRUE, group = "UPOV_ID",
    #              label = label.lines,
    ...
    validace <- validace %>% addCircleMarkers(data = stanice, lng = stanice@coords[,1], lat = stanice@coords[,2], 
                                               ...
                                               fillOpacity = 1, popup = label.points, layerId = dta_val$DBCN
     )}
...
if(input$validace == "A"){
        ...
        ts.dta.val <- dta_val %>% filter(DBCN == default.circle_val()) %>% select(input$entry.variable4, DTM)
        ...
        if( "QMER" %in% input$entry.variable4){
          p <- p %>% add_trace(x = ~DTM, y = ~QMER, name = 'QMER')} 
        
        if( "QNEX" %in% input$entry.variable4){
          p <- p %>% add_trace(x = ~DTM, y = ~QNEX, name = 'QNEX')}
        
        if("QNEY" %in% input$entry.variable4){
          p <- p %>% add_trace(x = ~DTM, y = ~QNEY, name = 'QNEY')}
        
        if("RM" %in% input$entry.variable4){
