...
cp <- eventReactive(input$go_cp,{
  
  ts.bilance <- BM %>% filter(UPOV_ID == default.shape(), year == input$year_cp, variable == input$variable_cp) %>%
...
absolutePanel(class = "panel panel-default", fixed = TRUE,
    ...
    HTML('<button data-toggle="collapse" data-target="#box5" class="choose">Výběr pro čáru překročení</button>'),
        tags$div(id = 'box5', class="collapse in", style = "max-height: 420px; overflow-y:scroll",
                 sliderInput("year_cp", "Zvolte:", 1961, 2015, step = 1, value = 2015, format="####"),
                 checkboxGroupInput("variable_cp", "Zvolte:", choices = choices, selected = c("P")),
                 actionButton("go_cp", "Spustit", icon("check"), style="color: #fff; background-color: #337ab7;
                                      border-color: #2e6da4")))
...
```{r}
renderDataTable({
  unit <- u %>% filter(NAZICO == default.circle()) %>% mutate(DTM = as.character(DTM)) %>% select(-c(X,Y, ROK, MESIC))}, options = list(scrollY = "400px", scrollX = "10px", class = 'cell-border stripe', lengthMenu = c(10,30,50,100)))
```
...
```{r}  
renderDataTable({
  subsubtotal <- u %>% filter(NAZICO %in% kde()$u_leaflet$NAZICO) %>% group_by(JEV) %>% replace(is.na(.), 0) %>% summarise(suma = sum(value))}, width='50%', options = list(scrollY = "400px", scrollX = "10px", 
...
```{r}
renderDataTable({
  subtotal <- u %>% filter(NAZICO %in% kde()$u_leaflet$NAZICO) %>% replace(is.na(.), 0) %>% group_by(NAZICO) %>% summarise(suma = sum(value))}, width='50%', options = list(scrollY = "400px", scrollX = "10px", 
                                                      lengthMenu = c(10,30,50,100)))
```
...
renderDataTable({
  ...
  total <- cbind("NAZICO" = "Suma", total)}, width='50%', options = list(scrollY = "400px", scrollX = "10px",
                                                                         lengthMenu = c(10,30,50,100)))
```
