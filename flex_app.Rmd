...
renderPlot({
  ...
  cp_c <- cp %>% filter(UPOV_ID == default.shape(), variable == input$variable_cp) %>%
 ...
 cp_s <- cp %>% group_by(seasons) %>% filter(UPOV_ID == default.shape(), variable == input$variable_cp) %>%
 ...
 cp_m <- cp %>% group_by(month2) %>% filter(UPOV_ID == default.shape(), variable == input$variable_cp) %>%
...
renderPlotly({
  ...
  ts.year <- dta_bd %>%  select(input$variable_trend, DTM) %>% filter(year(as.Date(dta_bd$DTM)) == year(as.Date(input$date_trend))) %>% mutate(m = month(as.Date(DTM)))
  colnames(ts.year)[1] <- "var" 
  ts.year$t <- (1:length(ts.year$var))

  ts.month <- dta_bd %>%  select(input$variable_trend, DTM) %>% filter(year(as.Date(DTM)) == year(as.Date(input$date_trend))&                                                                       month(as.Date(DTM))==month(as.Date(input$date_trend)))
...
ts1<-plot_ly(data=ts.year, x = ~t, y= ~var, name="Časová řada", mode="lines",type = 'scatter', text= paste(ts.year$DTM)) %>% 
...
ts2<- plot_ly(data=ts.month, x = ~t, y= ~var, name="Časová řada", mode="lines",type = 'scatter', text= paste(ts.month$DTM)) %>% 
  add_trace(y = ~lmt, name = 'Linearni trend', mode = 'lines') 
...
absolutePanel(class = "panel panel-default", fixed = TRUE,
                  draggable = FALSE, top = '65%', left = '0.5%', right = 'auto', bottom = 'auto',
                  width = '18%',
                  style = "opacity: 1; style = z-index: 400",
  wellPanel(
    HTML('<button data-toggle="collapse" data-target="#box6" class="choose">Výběr pro trendy</button>'),
        tags$div(id = 'box6', class="collapse in", style = "max-height: 420px",
                 ...
                 dateInput("date_trend", label = NULL, "yyyy-mm-dd", language = "cs",
                           value = as.Date("2010-01-01"), startview = "decade",
                           min = as.Date("1961-01-01"), max = as.Date("2015-12-01"))
                 )
    )
)
...
if(input$validace == "A"){
        ...
        plot_ly(data=ts.QD, x=~DTM, y=~value, text=paste(ts.QD$DTM), mode="lines", type="scatter") %>% layout(title=paste(as.character(title.f4())))
        
}else if(input$validace == "B"){
  
  
} else if(input$validace == "C" & !is.null(default.shape3())) {

     chars_filter <- data.frame(value = t(chars %>% filter(UPOV_ID == default.shape3()) %>% select(choices.mday)), m = c(seq(30,330,30),355,364))
   
   plot_ly(data=chars_filter, x=~m, y=~value, type="scatter", mode="lines") %>% layout(title=paste(as.character(title.f3.1())))
