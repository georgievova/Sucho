...
wellPanel(
  selectInput("entry.variable", label="Prvky hydrologické bilance",
            ...
            radioButtons("kartogram", label=NULL, choices = c("Dlouhodobé průměry" = "A", 
             "Měsíční průměry" = "B", "Roční průměry" = "C"), selected = "A"),
...
#                  
#                  conditionalPanel(condition = "input.kartogram == 'A'", 
#                                   selectInput("entry.variable", 
#                                               label="Prvky hydrologické bilance", 
#                                               choices = choices, selected = "P"),
#                                   selectInput("entry.year", label = "Zvolte rok:", 
...
reac<- eventReactive(input$go1,{ 
  ...
  bilance_filtr <- c()
  dta <- c()
  if(is.null(input$entry.year)){entry.year = 2015}else{entry.year=input$entry.year}
  if(is.null(input$entry.month)){entry.month = 01}else{entry.month=input$entry.month}
  
  if(input$DP == "DP1"){
    bilance_filtr <- bilance %>% filter(year==entry.year & month==entry.month)
