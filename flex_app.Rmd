...
BM <- collect(BM)
BM.long <- tbl(con, 'bil_monthly_long')
BM.long <- collect(BM.long)
u <- tbl(con, 'uzivani')
QD <- tbl(con, 'flow_rates_chmu')
...
```{r}
renderDygraph({
  ts.bilance <- reac()$bilance %>% filter(UPOV_ID == default.shape()) %>% group_by(year) %>% mutate(mean = mean(value)) %>% ungroup %>% select(value, mean)
...
reac2 <- eventReactive(input$go1,{ggpairs(BM.long[UPOV_ID == default.shape(),], columns = input$entry2.variables, mapping=aes(color=m, alpha = 0.25))})
