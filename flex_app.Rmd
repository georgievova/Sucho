...
output$map<-renderLeaflet({
  ...
  dta.flow <- dta
  tf <- routing.tab[dta.flow$UPOV_ID, default.shape()]==0
  if(any(tf)&any(!tf)){
    dta.flow[tf,"value"] <- NA
  }
  ...
  map <- leaflet() %>% 
    ...
    map <- map %>% addPolylines(data=reky, color="#007C8C", weight = 1.5,
          ...
          addPolylines(data=povodi_III, color="#cc0000",  weight = 2.5,
                       stroke= TRUE, group = "Povodí 3. řádu")
    
    
    map <- map %>% addPolygons(data=dta.flow, layerId = dta.flow$UPOV_ID,
                             group = "Horní povodí"
                 ) 
    ...
    map <- map  %>%
            addLayersControl(overlayGroups = c("Mapový podklad", "Řeky", "Jezera", "Nádrže", "Kraje", "Okresy", "Povodí","Horní povodí", "Povodí 3. řádu"),
                     position = "bottomleft",
   ...
   # zvyraznění zvoleného polygonu
   # lepší způsob?
   # map <- map %>% addPolygons(data = dta[dta$UPOV_ID == default.shape(),], 
   #                            color = "#a53333", opacity = 1.5, fillOpacity = 0)
...
#                 width = '15%', height = '4%', style = "opacity: 1; style = z-index: 400",
#                 selectInput(inputId = "canton", "Přibliž na:", choices = ))
