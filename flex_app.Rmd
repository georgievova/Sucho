...
renderPlotly({
  ...
  routed0_dir <- data.frame(source=d, UPOV_ID=gsub("*.rds","",d))
  routed <- readRDS(file.path(.datadir,"routed-0_bilan_bez_VN",
                                   routed0_dir$source[routed0_dir$UPOV_ID==default.shape3()]))
  routed <- routed[,c("DTM", "TRM_mm_d")]
  routed.qm <- CatCa::mdq(routed$TRM_mm_d, return = 'data.table')
   
   p2 <- plot_ly(type="scatter", mode="lines", text=paste(routed.qm$LAB)) %>%
     layout(title=paste(as.character(title.f3.1())))
   
   p2 <- p2 %>% add_trace(data=chars_filter, x=~m, y=~value, name = 'Pozorovana data')
   p2 <- p2 %>% add_trace(data=routed.qm, x=~M, y=~qm, name = 'Simulovana data')
   p2
  }
})
```
...
#   print(cbind(name=rownames(pars_filter),pars_filter[,-1]), digits=3)
  ...
  routed <- readRDS(file.path(.datadir,"routed-0_bilan_bez_VN",
                                   routed0_dir$source[routed0_dir$UPOV_ID==default.shape3()]))
  routed <- routed[,c("DTM", "TRM_mm_d")]
  routed.qm <- CatCa::mdq(routed$TRM_mm_d, return = 'vector')
  
  table <- rbind(chars_filter, routed.qm)
  }
}, digits = 3, spacing = 'xs')
```
