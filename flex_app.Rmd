...
BM <- readRDS(file.path(.datadir, "webapp_data/mbilan/bilan_month_dt_2018.rds"))
...
#Roční průměry
    ...
    bilance_filtr <- bilance[year==input$entry.yearC,]
    bilance_filtr <- bilance_filtr[!duplicated(UPOV_ID),]
    colnames(bilance_filtr)[colnames(bilance_filtr)=="value"] <- "monthly.avg"
...
map <-  map %>% addFeatures(data=dta, layerId = dta$UPOV_ID, color = "black",
...
# 
    ...
    if("R" %in% vrstvy){ #addPolylines
       map <- map %>% addFeatures(data=reky, color="#007C8C", weight = 1.5,
                 opacity = 1, stroke= TRUE, group = "Řeky")
       }

    if("J" %in% vrstvy){ #addPolygons
       map <- map %>% addFeatures(data=jezera, color="#007C8C", fillColor = "#0099ff",
                weight = 1, opacity = 1, stroke= TRUE, group = "Jezera")}

    if("N" %in% vrstvy){ #addPolygons
      map <-  map %>% addFeatures(data=nadrze, color="#007C8C", fillColor = "#0099ff",
                weight = 1, opacity = 1, stroke= TRUE, group = "Nádrže")}

    if("K" %in% vrstvy){ #addPolylines
      map <-  map %>% addFeatures(data=kraje, color="#000000",  weight = 2.5, fill=F,
                       stroke= TRUE, group = "Kraje")}

    if("O" %in% vrstvy){ #addPolylines
       map <- map %>% addFeatures(data=okresy, color="#000000",  weight = 2, fill=F,
...
#Zbarvení povodí dle hodnoty
   ...
   if("P" %in% vrstvy){ #addPolygons
       indikatory <- indikatory %>% addFeatures(data=dta, layerId = dta$UPOV_ID, color = "black",
                             weight = 1.5, opacity = 0.45, smoothFactor = 0,
    ...
    if("R" %in% vrstvy){ #addPolylines
       indikatory <- indikatory %>% addFeatures(data=reky, color="#007C8C", weight = 1.5,
                 opacity = 1, stroke= TRUE, group = "Řeky")
       }

    if("J" %in% vrstvy){ #addPolygons
       indikatory <- indikatory %>% addFeatures(data=jezera, color="#007C8C", fillColor = "#0099ff",
                weight = 1, opacity = 1, stroke= TRUE, group = "Jezera")}

    if("N" %in% vrstvy){ #addPolygons
      indikatory <-  indikatory %>% addFeatures(data=nadrze, color="#007C8C", fillColor = "#0099ff",
                weight = 1, opacity = 1, stroke= TRUE, group = "Nádrže")}

    if("K" %in% vrstvy){ #addPolylines
      indikatory <-  indikatory %>% addFeatures(data=kraje, color="#000000",  weight = 2.5, fill=F,
                       stroke= TRUE, group = "Kraje")}

    if("O" %in% vrstvy){ #addPolylines
       indikatory <- indikatory %>% addFeatures(data=okresy, color="#000000",  weight = 2, fill=F,
                       stroke= TRUE, group = "Okresy")}
...
# leaflet() %>% setView(lng=centr[,1], lat = centr[,2], zoom=12) %>%
  ...
  if("R" %in% vrstvy){ #addPolylines
  uzivani <- uzivani %>% addFeatures(data=reky, color="#007C8C", weight = 1.5,
                 opacity = 1, stroke= TRUE, group = "Řeky") }
  
  if("J" %in% vrstvy){ #addPolygons
  uzivani <- uzivani %>% addFeatures(data=jezera, color="#007C8C", fillColor = "#0099ff",
                weight = 1, opacity = 1, stroke= TRUE, group = "Jezera") }
  
  if("N" %in% vrstvy){ #addPolygons
  uzivani <- uzivani %>% addFeatures(data=nadrze, color="#007C8C", fillColor = "#0099ff", 
                weight = 1, opacity = 1, stroke= TRUE, group = "Nádrže") }
  ...
  if("P" %in% vrstvy){ #addPolygons
   uzivani <- uzivani %>% addFeatures(data=povodif, color = "#000000", fillColor = "#708090", weight = 2, opacity = 0.4, smoothFactor = 0,
...
# leaflet() %>% setView(lng=centr[,1], lat = centr[,2], zoom=12) %>%
  ...
  if("O" %in% vrstvy){ #addCircleMarkers
   vyhled_uzivani <- vyhled_uzivani %>% addCircleMarkers(data=u_leaflet, radius = 6.5, color = "black", 
    ...
    if("P" %in% vrstvy){
    vyhled_uzivani <- vyhled_uzivani %>% addFeatures(data=povodiv, color = "#000000", fillColor = "#708090", weight = 2, opacity = 0.4, smoothFactor = 0,
                                       layerId = povodi$UPOV_ID,
                                       label = labels,
                                       labelOptions = labelOptions(clickable = TRUE, noHide = FALSE,
                                                                   style = list("font-weight" = "normal",
...
output$validace <- renderLeaflet({
  ...
  validace <- leaflet()
    ...
    if("P" %in% vrstvy){ #addPolygons
   validace <- validace %>% addFeatures(data=dta,
    ...
    if("P" %in% vrstvy){ #addPolygons
     validace <- validace %>% addFeatures(data=dta, layerId = dta$UPOV_ID, 
                                             color = "burlywood4", weight = 1.3, smoothFactor = 0,
                                             opacity = 0.3, fillColor = ~pal(value),
                                             fillOpacity = 0.8, group = "Povodí",
                                highlightOptions = highlightOptions(color = "#800000", opacity = 0.8,
  ...
  #Vrstvy mapy
    ...
    if("R" %in% vrstvy){ #addPolylines
       validace <- validace %>% addFeatures(data=reky, color="#007C8C", weight = 1.5,
                 opacity = 1, stroke= TRUE, group = "Řeky")}

    if("J" %in% vrstvy){ #addPolygons
       validace <- validace %>% addFeatures(data=jezera, color="#007C8C", fillColor = "#0099ff",
                weight = 1, opacity = 1, stroke= TRUE, group = "Jezera")}

    if("N" %in% vrstvy){ #addPolygons
      validace <-  validace %>% addFeatures(data=nadrze, color="#007C8C", fillColor = "#0099ff",
                weight = 1, opacity = 1, stroke= TRUE, group = "Nádrže")}

    if("K" %in% vrstvy){ #addPolylines
      validace <-  validace %>% addFeatures(data=kraje, color="#000000",  weight = 2.5, fill = F,
                       stroke= TRUE, group = "Kraje")}

    if("O" %in% vrstvy){ #addPolylines
       validace <- validace %>% addFeatures(data=okresy, color="#000000",  weight = 2, fill = F,
                       stroke= TRUE, group = "Okresy")}
