...
#
renderUI({
  wellPanel(selectizeInput(inputId = "search.id","Vyhledávání útvaru",
                           selected=input$map_shape_click$id,
                           choices = search.choices),HTML('<div class="help-tip">
...
wellPanel(
  selectInput("entry.variable", label="Prvky hydrologické bilance",
            ...
            radioButtons("kartogram", label=NULL, choices = c("Dlouhodobé průměry" = "A", 
             "Měsíční průměry" = "B", "Roční průměry" = "C", "Filtrování dle hodnoty" = "D"),
             selected = "A"),
          ...
          renderUI({sliderInput("entry.range", label = "Filtrace dle hodnoty",
...
renderPlot({  
  ts.bilance <- BM %>% filter(UPOV_ID == default.shape(), variable == input$variable_cp) %>%
   ...
   ggt <- ggplot(ts.bilance, aes(1:nrow(ts.bilance), value))+geom_line()+xlab("prob")+ylab(input$variable_cp)+theme_minimal()
  ...
  ts.bilance <- ts(ts.bilance, frequency = 12, start=c(1961,1))
    ...
    ts.s$rank <- 0
    for(i in ts.s$s){ts.s$rank[ts.s$s==i] <- order(-ts.s$value[ts.s$s==i])}
    ggs <- ggplot(ts.s, aes(rank, value))+geom_line(aes(group=s, color=s))+xlab("prob")+ylab(input$variable_cp)+theme_minimal()
    ...
    ts.m$rank <- 0
    for(i in ts.m$m){ts.m$rank[ts.m$m==i] <- order(-ts.m$value[ts.m$m==i])}
    ggm <- ggplot(ts.m, aes(rank, value))+geom_line(aes(group=m, color=m))+xlab("prob")+ylab(input$variable_cp)+theme_minimal()
