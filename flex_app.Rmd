...
renderUI({
    absolutePanel(class = "panel panel-default", fixed = TRUE,
                draggable = TRUE, top = '5%', left = '4%', right = "auto", bottom = "auto",
                width = '26%', style = "opacity: 1; style = z-index: 400",
      wellPanel(
...
output$validace <- renderLeaflet({
  ...
  validace <- leaflet() %>% setView(lat = initial_lat, lng = initial_lng, zoom = initial_zoom)
    ...
    if(input$validace == "A"){
      ...
      if("P" %in% vrstvy){
      validace <- validace %>% addPolylines(data=QD_stanice, color="#CA5557", weight = 2.3, dashArray = "3",  fill = TRUE,
...
#     #clearGroup("current_shape") %>% 
#     # addPolylines(data = subset(povodi,
#     #                            UPOV_ID==stanice$UPOV_ID[stanice$DBCN==default.circle_val()]),
#     #    #          group = "current_shape33",
#     #              color = "#a53333", opacity = 1, weight = 3,
#     #              stroke = TRUE) 
# }
# })
...
if(input$validace == "A"){
  ...
  dta_val_filter <- dta_val %>% filter(DBCN == default.circle_val()) %>% select(QMER, QNEX, QNEY, RM)
  dta_val_filter <- psych::describe(dta_val_filter, skew = F, IQR = T, quant = c(0.5, 0.75))
