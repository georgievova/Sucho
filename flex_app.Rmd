...
wellPanel(
  selectInput("entry.variable", label="Prvky hydrologické bilance",
          ...
          renderUI({sliderInput("entry.range", label = "Filtrace dle hodnoty",
                  min = reac()$entry_min, 
                  max = round(reac()$entry_max),
                  value = c(reac()$entry_min,
                            round(reac()$entry_max)))
            })
  )
...
output$map<-renderLeaflet({
  ...
  map <- map %>% addPolygons(data=dta, layerId = dta$UPOV_ID, color = "burlywood4", weight = 1.3,
                 ...
                 highlightOptions = highlightOptions(color = "#a53333", opacity = 1, 
                                                     fillOpacity = 0,
                                                     weight = 2.3,
    ...
    addLayersControl(overlayGroups = c("Povodí","Mapový podklad", "Řeky", "Jezera", "Nádrže", "Kraje", "Okresy", "Povodí 3. řádu"),
                     position = "bottomleft",
