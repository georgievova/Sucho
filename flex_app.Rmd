...
renderDygraph({
  ts.uzivani <-  u[NAZICO == default.circle()] %>% group_by(JEV, DTM) %>% ungroup %>% dcast(DTM~JEV)
  ts.uzivani <- xts::xts(ts.uzivani, order.by = ts.uzivani$DTM)
  #ts.uzivani <- ts(ts.uzivani, start = decimal_date(as.Date("2006-12-01")), frequency = 1)
...
renderDygraph({
  ts.uzivani.uv <-  u[NAZICO == default.circle_uv()] %>% group_by(JEV, DTM) %>% ungroup %>% dcast(DTM~JEV)
