...
BM <- readRDS(file.path(.datadir, "webapp_data/bilan_n/bilan_month_dt.rds"))

BM_week <- readRDS(file.path(.dirM, "week_stable.rds"))

povodi <- readRDS(file.path(.datadir, "webapp_data/st_read_rds/povodi.rds"))
reky <- readRDS(file.path(.datadir, "webapp_data/st_read_rds/reky.rds"))
jezera <- readRDS(file.path(.datadir, "webapp_data/st_read_rds/jezera.rds"))
nadrze <- readRDS(file.path(.datadir, "webapp_data/st_read_rds/nadrze.rds"))
kraje <- readRDS(file.path(.datadir, "webapp_data/st_read_rds/kraje.rds"))
okresy <- readRDS(file.path(.datadir, "webapp_data/st_read_rds/okresy.rds"))
povodi_III <- readRDS(file.path(.datadir, "webapp_data/geo_rds/povodi_III.rds"))
# povodi_III <- readRDS(file.path(.datadir, "webapp_data/st_read_rds/povodi_III.rds"))


# QD_stanice <- readRDS(file.path(.datadir, "webapp_data/st_read_rds/stanice.rds"))
...
u <- readRDS(file.path(.datadir, "webapp_data/uzivani/uzivani_06_17_dt.rds"))
u_leaflet <- readRDS(file.path(.datadir, "webapp_data/uzivani/u_leaflet.rds"))
u_pas <- readRDS(file.path(.datadir, "webapp_data/uzivani/uzivani_pas.rds"))
...
wellPanel(HTML('<div class="help-tip">
  ...
  conditionalPanel(condition = "input.kartogram == 'B'",
                 ...
                 dateInput("entry.date", label = NULL, "yyyy-mm-dd", language = "cs",
                           value = as.Date("2018-06-01"), startview = "decade",
                           min = as.Date("1982-11-01"), max = as.Date("2018-06-01"))
                 ),
...
renderDygraph({
  ...
  ts.bilance <- BM_week[BM_week$IID == default.shape() & BM_week$variable == input$entry.variable3,] %>% select(value, DTM) 
...
absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE,
    ...
    HTML('<button data-toggle="collapse" data-target="#box4" class="choose">Výběr časových řad</button>'),
        tags$div(id = 'box4', class="collapse", style = "max-height: 250px; overflow-y:scroll",
                 checkboxGroupInput("entry.variable3", "Zvolte:", choices = choices, selected = c("RM"))))
...
wellPanel(
  radioButtons("indikatory", label=NULL, choices = c("SGI" = "sgi", 
                                                     "SPEI" = "spei", "SPI" = "spi", 
                                                     "PDSI" = "pdsi"),
               selected = "sgi")
...
wellPanel(
    dateInput("entry.date.ind", label = NULL, "yyyy-mm-dd", language = "cs",
                           value = as.Date("2019-03-18"), startview = "year",
                           min = as.Date("2010-01-04"), max = as.Date("2019-03-18"))
)
...
reac.ind <- eventReactive(input$go2,{ 
 ...
 ind_dta <- readRDS(file.path(.datadir,"webapp_data/indikatory_new", paste0(input$indikatory, ".rds")))
 colnames(ind_dta)[3] <- "value"

 dta <- (sp::merge(povodi, 
                   ind_dta[ind_dta$DTM == input$entry.date.ind, ],
                   by="UPOV_ID"))
...
observeEvent(input$send, {
  ...
  write.table(changes(), file.path(.datadir, "webapp_data/icoc_values", 
                                   paste0(u_leaflet$ICOC[u_leaflet$NAZICO ==input$uzivani_marker_click$id],
                                          ".csv")))
...
renderDygraph({
  ts.uzivani <-  u[UPOV_ID == default.shape2()] %>% group_by(JEV, DTM) %>%
                       summarise(sum = sum(value)) %>% ungroup %>% dcast(DTM~JEV, value.var = "sum")
...
renderDataTable({
  subtotal <- u[UPOV_ID == default.shape2()] %>% replace(is.na(.), 0) %>% group_by(NAZICO, JEV) %>% summarise(suma = sum(value)) %>% select(NAZICO, JEV, suma)}, class = 'compact cell-border', colnames = c("Odběratel", "Jev", "Suma"), options = list(scrollY = "300px", lengthMenu = c(100,200), language = list(search = 'Hledat:', lengthMenu = 'Zobrazit_MENU_položek')))
```
...
default.circle_uv <- reactive({
  ...
  NAZICO.sel <- u_leaflet$NAZICO[109]}
...
renderTable({
u_pas_ic <- u_pas %>% filter(NAZICO == default.circle_uv())
c <- colnames(u_pas_ic)
u_pas_ic[2,] <- u_pas_ic[1,]
u_pas_ic[1,] <- c
if(nrow(u_pas %>% filter(NAZICO == default.circle_uv())) > 0){
u_pas_ic <- t(u_pas_ic)
...
output$hotable_v <- renderRHandsontable({
  u_val1 <- u %>% filter(NAZICO == default.circle_uv(), ROK == 2017) %>% select(MESIC, value) %>%
                  filter(MESIC %in% unique(as.character(month(date[1:4])))) %>% summarise(value=mean(value))
  # 
  u_val2 <- u %>% filter(NAZICO == default.circle_uv(), ROK == 2017) %>% select(MESIC, value) %>%
...
wellPanel(
  br(),
  HTML(paste("<b>Odebrané množství 2017</b>")),
  tableOutput("odbery"),
  br(),
