...
renderPlot({
  ...
  #rocni data
  dta_bd <- BM[UPOV_ID == default.shape() &
                 (year > input$date_trend[1] & year < input$date_trend[2]) & 
                 variable == input$variable_trend]
  
  ts.year <- dta_bd[!duplicated(year),] %>% select(annual.avg, year)
  ...
  ts1 <- ggplot(ts.year) + 
    ...
    geom_line(aes(y=lmt, x=year), color = 'darkblue', show.legend = F)+
    theme_minimal()
  
  ts.month <- dta_bd %>% select(value, month2, DTM)
  colnames(ts.month)[1] <- "var"
  ts.month$t <- (1:length(ts.month$var))
  
  model_m <<- lm(var~t,data=ts.month)
  ts.month$lmt <- model_m$fitted.values
  
  ts2 <- ggplot(ts.month) + 
    geom_line(aes(x=DTM, y=var)) +
    labs(x = 'DTM', y = paste(input$variable_trend), title = 'Měsiční průměry') +
    geom_line(aes(y=lmt, x=DTM), color = 'darkblue', show.legend = F)+
    theme_minimal()
  ...
  ts.season <- dta_bd %>% select(seasons, value, DTM) %>%  filter(year(DTM) > input$date_trend[1] & year(DTM) < input$date_trend[2])
  ts.season$t <- (1:length(ts.season$value))
  
  model_s <<- lm(value~t+as.factor(seasons),data=ts.season)
  ...
  trend_seasons <- ggplot(ts.season) + 
    geom_line(aes(x=DTM, y=value, color = seasons)) + 
    facet_wrap(~seasons)+
    labs(x = 'DTM', y = paste(input$variable_trend)) +
    geom_line(aes(x=DTM, y = lmt), color = 'darkblue') +
    theme_minimal()

ts <- gridExtra::grid.arrange(ts1, ts2, ncol=1)
gridExtra::grid.arrange(ts, trend_seasons, ncol= 2)
...
renderTable({
  ...
  sum.tab <- function(smr){
  smr <- summary(smr)
  df <- data.frame("intercept" = smr$coefficients[1,1], "směrnice" = smr$coefficients[2,1], "významnost" = smr$coefficients[2,4], "R2" = smr$r.squared, row.names = "ROK")
  return(df)}
  
  rbind(sum.tab(model_r), sum.tab(model_m), sum.tab(model_s))
