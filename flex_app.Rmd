...
output$odbery <- renderTable({
  ...
  # u_povol <- u_pas %>% filter(NAZICO == default.circle_uv()) %>% select(PMM3_ROK, PMM3_MES) %>% mutate(RM=sum(u_odber$value))
  
  if(nrow(u_odber) > 0){
  # u_povol <- data.frame(MESIC = colnames(u_povol), value = c(u_povol$PMM3_ROK, u_povol$PMM3_MES, u_povol$RM))
...
tab <- as.data.frame(t(data.frame(hodnota = rep(0,8), 
                                  datum = seq(from = Sys.Date(), by = "weeks", length.out = 8), 
                                  row.names = 1:8)))
...
output$hotable_v <- renderRHandsontable({
  ...
  rhandsontable(hnds_table_v(), stretchH = "all", height = 130) %>% hot_row(2, readOnly = TRUE)# %>% 
#     hot_cols(render = "
