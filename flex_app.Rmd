---
title: "`r HTML('<div style=width:370px;text-align:center>HAMR</div>')`"
output: 
...
```{r global, context = "setup", include=FALSE}
...
# BM <- readRDS('data/mbilan/bilan_month_ep.rds')
...
CatCa::Data("webapp_data")
povodi_III = rgdal::readOGR(file.path(.datadir, "webapp_data/geo/povodi_III.shp"))
...
#Vytvaření palet
  ...
  if(input$entry.variable == "T"){
        pal <- colorBin(mojePaleta, domain = dta$value, pretty = TRUE, reverse = TRUE)

  }else{pal <- colorBin(mojePaleta, domain = dta$value, pretty = TRUE, 
                        na.color = "#595959")
...
absolutePanel(id = "logos", fixed = TRUE,
                  draggable = FALSE, top = "auto", left = "auto", right = "auto", bottom = 1,
                  width = 385,
                  style = "opacity: 1;",
 wellPanel(HTML('<img src="img/logo_vuv.png" style="height:50px" hspace="6" />'),
           HTML('<img src="img/logo_fzp.png" style="height:50px" hspace="6" />'),
           HTML('<img src="img/logo_cgz.png" style="height:50px" hspace="6" />'),
           HTML('<img src="img/logo_chmi.png" style="height:50px" hspace="6" />')))
```
...
```{r cr mesicni data, context="server"}
...
```{r cr denni data, context="server"}
renderDygraph({
...
```{r cara prekroceni, context="server"}
...
wellPanel(HTML('<div class="help-tip">
               <p>Volba indikátoru a kroku. Indexy jsou počítány klouzavě s krokem 1 týden, 1, 3, 6 a 12 měsíců. Rozdělujeme do 4 kategorií dle hodnot indexu:

0 - bez výskytu sucha (-0,7 a vyšší)
1 - slabé sucho (-0,8 až -1,3)
2 - silné sucho (-1,4 až -1,9)
3 - mimořádné sucho (- 2,0 a nižší) </p></div>'),

  radioButtons("indikatory", label=NULL, choices = c("SPI" = "A", 
             "SPEI" = "B", "PDSI" = "C", "Nedostatkové objemy" = "D"),
             selected = "A"),
  selectInput("krok", label = NULL, choices = c("1 týden" = "1t", "1 měsíc" = "1m", "3 měsíce" = "3m",
                                                "6 měsíců" = "6m", "12 měsíců" = "12m"), selected = "3m"))
...
```{r uzivani: abs.panel, context="server"}

renderUI({
    absolutePanel(class = "panel panel-default", fixed = TRUE,
                draggable = FALSE, top = 30, left = 15, right = "auto", bottom = "auto",
                width = '15%', height = '4.5%', style = "opacity: 1; style = z-index: 400",
...
```{r uzivani: casova rada, context="server"}
renderDygraph({
...
```{r uzivani: tab:odberatel, context="server"}
renderDataTable({
...
```{r uzivani: TS odberatel, context="server"}
renderDygraph({
  ts.uzivani <-  u %>% filter(NAZICO == default.circle()) %>% group_by(JEV, DTM) %>%
                       summarise(sum = sum(value)) %>% ungroup %>% dcast(DTM~JEV)
  ts.uzivani <- xts::xts(ts.uzivani, order.by = ts.uzivani$DTM)
  #ts.uzivani <- ts(ts.uzivani, start = decimal_date(as.Date("2006-12-01")), frequency = 1)
...
```{r validace: mapa, context="server"}
...
output$rozvodnice <- renderLeaflet({
  ...
  rozvodnice <- leaflet() %>%
    ...
    else if(input$validace == "B"){pars_filtr <- pars %>% filter(name == input$entry.pars)
        colnames(pars_filtr)[colnames(pars_filtr) == "current"] <- "value"
     dta <- (sp::merge(povodi,pars_filtr, by="UPOV_ID"))
   
     labels <- sprintf(
     "<strong>%s</strong><br/>current: %s <br/> lower: %s <br/> upper: %s <br/> initial: %s",
      dta$UPOV_ID, dta$value, dta$lower, dta$upper, dta$initial
      ) %>% lapply(htmltools::HTML)
...
```{r QD: casova rada, context="server"}

renderDygraph({
  ts.QD <- QD %>% filter(DBCN == default.line()) %>% mutate(month = format(DTM, "%m"), year = format(DTM, "%Y")) %>% 
  group_by(year) %>% mutate(mean_year = mean(value, na.rm = TRUE)) %>% ungroup %>% group_by(month, year) %>% mutate(mean_month = mean(value, na.rm=TRUE)) %>% ungroup %>% select(input$entry.variable4, DTM)
...
absolutePanel(class = "panel panel-default", fixed = TRUE,
                  draggable = FALSE, top = 80, left = "auto", right = 10, bottom = "auto",
                  width = '10%',
                  style = "opacity: 1; style = z-index: 400",
    HTML('<button data-toggle="collapse" data-target="#box8" class="choose">Výběr časových řad</button>'),
        tags$div(id = 'box8', class="collapse", style = "max-height: 250px; overflow-y:scroll",
...
absolutePanel(class = "panel panel-default", fixed = TRUE,
                draggable = FALSE, top = 100, left = 80, right = "auto", bottom = "auto",
                width = '8%', height = '7.5%', style = "opacity: 1; style = z-index: 400",
