...
actionButton("go", "Spustit", icon("check"), 
...
absolutePanel(bottom = 20, right = 20, width = 420,
  ...
  wellPanel(
  tags$div(class = "row",
    checkboxGroupInput("entry2.variables", "Zvolte:", choices = choices, selected = c("P", "PET", "T")),
...
reac2 <- eventReactive(input$go1,{ggpairs(BM.long[BM.long$UPOV_ID == default.shape(),], columns = input$entry2.variables, mapping=aes(color=m, alpha = 0.2))})
...
# 
# actionButton("go2", "Spustit", icon("check"),
#     style="color: #fff; background-color: #337ab7; border-color: #2e6da4")
```
