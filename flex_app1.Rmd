---
title: "`r HTML('<div style=width: 75%> Demo </div>')`"
output: 
...
u <- readRDS('data/uzivani_ocistene.rds')
...
wellPanel(selectInput("entry.variable", label="Prvky hydrologické bilance", choices = choices, selected = "P"))


wellPanel(HTML('<button data-toggle="collapse" data-target="#box1" class="btn-block btn-primary">Dlouhodobé průměry</button>'),
        tags$div(id = 'box1', class="collapse in"),

        HTML('<button data-toggle="collapse" data-target="#box2" class="btn-block btn-primary">Měsíční průměry</button>'),
...
renderDygraph({
  # ts.bilance <- BM.long %>%  filter(UPOV_ID == default.shape()) %>% group_by(year) %>% 
  #   mutate(mean = mean(input$entry.value2)) %>% ungroup %>% select(input$entry.variable2, mean)
...
absolutePanel(bottom = 0, right = 20, width = 350, draggable = FALSE,
  wellPanel(
    HTML('<button data-toggle="collapse" data-target="#box4" class="btn-block btn-primary">Výběr časových řad</button>'),
        tags$div(id = 'box4', class="collapse in",
                 checkboxGroupInput("entry.variable2", "Zvolte:", choices = choices2, selected = c("PET", "ET")))))
...
```{r}
absolutePanel(bottom = 20, right = 20, width = 420,
    draggable = FALSE,
  wellPanel(
  tags$div(class = "row",
    checkboxGroupInput("entry.variable3", "Zvolte:", choices = choices, selected = c("P", "PET", "T")),
...
reac3 <- eventReactive(input$go3,{ggpairs(BM.long[BM.long$UPOV_ID == default.shape(),], columns = input$entry.variable3, mapping=aes(color=m, alpha = 0.2))})
...
```{r}
renderDygraph({
  ts.QD <-  kde()$u %>% filter(kde()$u$NAZICO == default.circle()) %>%  select(value)
...
```{r}  
renderTable({
  subsubtotal <- kde()$u %>% group_by(JEV) %>% select(DEL, RKM, RKMO, RM, RH, PMM3_ROK, PMM3_MES, Q_MZP, OD_ZAVL, OD_VER_V, MN_DODANE) %>% replace(is.na(.), 0) %>% summarise_all(funs(sum))})
```
...
```{r}
renderTable({
  unit <- kde()$u %>% filter(kde()$u$NAZICO == default.circle()) %>%  select(NAZICO, DTM, DEL, RKM, RKMO, RM, RH, PMM3_ROK, PMM3_MES, Q_MZP, OD_ZAVL, OD_VER_V, MN_DODANE)})
```
...
output$rozvodnice <- renderLeaflet({
  ...
  labels.lines <- sprintf(
      "<strong>%s</strong><br/> DBCN:%s <br/> Plocha [km^2]: %s",
