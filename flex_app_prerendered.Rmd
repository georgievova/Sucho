...
renderUI({
  wellPanel(selectizeInput(inputId = "search.id","Vyhledávání útvaru",selected=input$map_shape_click$id,
...
wellPanel(
        ...
        tags$div(id = 'box2', class="collapse in", 
                 radioButtons("kartogram", label=NULL, 
                    choices = c("Přepinání parametrů" = "C", "Měsíční průměry" = "A", "Celé období" = "B"),
                    selected = "B"),
                 ...
                 conditionalPanel(condition = "input.kartogram == 'A'", 
                                  ...
                                  selectInput("entry.year", label = "Zvolte rok:", 
                                              choices = seq(1961,2015,1), selected = 2015),
                ...
                conditionalPanel(condition = "input.kartogram == 'C'",
                                  selectInput("entry.pars", label=NULL, 
                                              choices = pars$name, selected = "Spa"))),
        ...
        tags$div(id = 'box3', class="collapse",
                 radioButtons("filtr", "Filtr", 
                              choices = c("Kartogram" = "A", "Filtrovat podle hodnoty" = "B")),
...
reac<- eventReactive(input$go1,{ 
  ...
  if(input$kartogram == "A"){
    ...
    dta <- (sp::merge(povodi,bilance_filtr, by="UPOV_ID"))
  }else if(input$kartogram == "C"){pars_filtr <- pars %>% filter(name == input$entry.pars)
  ...
  return(list(bilance=bilance_filtr,dta=dta))
...
```{r loga}
# absolutePanel(WellPanel(id = "logos", class = "panel panel-default", fixed = TRUE,
#                   draggable = FALSE, top = "auto", left = "auto", right = "auto", bottom = "auto",
#                   width = "auto",
#                   style = "opacity: 0.9; style = z-index: 400",
#     HTML('<img src="img/logo_vuv.png" style="height:50px" />'),
#     HTML('<img src="img/logo_fzp.jpg" style="height:50px" />'),
#     HTML('<img src="img/logo_cgz.png" style="height:50px" />'),
#     HTML('<img src="img/logo_chmi.png" style="height:50px" />')))
```
...
output$map<-renderLeaflet({
  ...
  pal_1 <- colorNumeric("#000066", domain = NULL, na.color = "gray50") 
  ...
  if (input$filtr == "A") {
  map <- map %>% addPolygons(data=dta, layerId = dta$UPOV_ID, color = "burlywood4", weight = 1.3,
                                                                                    opacity = 0.3,
                 fillColor = ~pal(value), fillOpacity = 0.8, group = "Povodí",
                 highlightOptions = highlightOptions(color = "#a53333", opacity = 1, fillOpacity = 0,
                                                     weight = 2.3,
    ...
    addLayersControl(overlayGroups = c("Povodí","Mapový podklad", "Řeky", "Jezera", "Nádrže", "Kraje", "Okresy"),
                     position = "bottomleft",
                     options = layersControlOptions(collapsed = FALSE, autoZIndex = FALSE)) %>% 
    hideGroup(c("Kraje", "Okresy"))%>%
    addLegend(pal = pal, values = dta$value, opacity = 0.7, title = "Legenda",
              position = "bottomright")
  }else if (input$filtr == "B") {
  map <- map %>% addPolygons(data=dta1, layerId = dta1$UPOV_ID, color = "#ffffff", 
                             fillColor = ~pal_1(dta1$value), 
                 ...
                 labelOptions = labelOptions(clickable = TRUE, style = list("font-weight" = "normal",
                                             ...
                                             "border-color" = "rgb(255, 255, 255)"),
                                             textsize = "15px", direction = "auto")) %>%
          addPolygons(data=dta2, layerId = dta2$UPOV_ID, color = "#ffffff", fillColor = ~pal_1(dta2$value), 
          ...
          addLayersControl(baseGroups = c("Vetsi nebo rovno", "Mensi nez"),
                     overlayGroups = c("Mapový podklad", "Řeky", "Jezera", "Nádrže", "Kraje", "Okresy"),
                     position = "bottomleft",
                     options = layersControlOptions(collapsed = FALSE, autoZIndex = FALSE)) %>%   
          hideGroup(c("Řeky", "Jezera", "Nádrže", "Kraje", "Okresy"))
  }
    ...
    map <- map %>% addPolylines(data=reky, color="#007C8C", weight = 1.5,
          ...
          addPolygons(data=kraje, color="#000000", fillColor = "#595959", 
                weight = 2, opacity = 1, stroke= TRUE, group = "Kraje",
                popup = kraje$NAZEV) %>%
          addPolygons(data=okresy, color="#000000", fillColor = "#595959", 
                weight = 1, opacity = 1, stroke= TRUE, group = "Okresy",
                popup = okresy$NAZEV)
})
...
kde <- reactive({
u_leaflet <- u %>% filter(UPOV_ID == default.shape2()) %>% group_by(UPOV_ID, NAZICO, POVODI, JEV, X, Y, ICOC) %>% summarise(mean=mean(value))
...
labels.points <- sprintf(
  ...
  if(is.null(u_leaflet$JEV)){pal_2 <- NULL
  }else{pal_2 <- colorFactor("Paired", domain = u_leaflet$JEV, na.color = "gray50")}
  
  centr <- getSpPPolygonsLabptSlots(povodi.ds())

uzivani <- leaflet() %>%setView(lng=centr[,1], lat = centr[,2], zoom=12) %>%
    ...
    addPolygons(data=povodif, color = "#000000", fillColor = "#708090", weight = 2, opacity = 0.4,
                          fillOpacity = 0.7, layerId = povodif$UPOV_ID,
                          highlightOptions = highlightOptions(color = "	#DC143C", opacity = 1, 
                                                              fillOpacity = 0, weight = 2.3,
                                                     bringToFront = TRUE, sendToBack = TRUE),
                label = labels,
                labelOptions = labelOptions(clickable = TRUE, noHide = TRUE,
                                            style = list("font-weight" = "normal",
                                              ...
                                              "border-color" = "#F5F5F5"),
                                              textsize = "15px", direction = "auto")) %>%
    addCircleMarkers(data=xy, radius = 6.5, color = "black", fillColor = pal_2(u_leaflet$JEV),
...
#   labels <- sprintf(
#       "<strong>%s</strong><br/>Kategorie: %s<br/>Stav: %s <br/> ID: %s",
#       dta$NAZ_UTVAR, dta$KTG_UPOV, dta$U_PMU, dta$UPOV_ID) %>% lapply(htmltools::HTML)
# 
# labels.points <- sprintf(
#       "<strong>%s</strong><br/>Povodí: %s<br/>Jev: %s<br/>ICOC: %s",
#       u_leaflet$NAZICO, u_leaflet$POVODI, u_leaflet$JEV, u_leaflet$ICOC) %>%
...
renderDygraph({
  ts.uzivani <-  u %>% filter(UPOV_ID == input$search.id.u) %>% group_by(JEV, DTM) %>%
  ...
  dygraph(ts.uzivani, main = paste(as.character(title.f())))
})
```
...
renderDygraph({
  ...
  dygraph(ts.QD, main = paste(as.character(title.f3())), 
